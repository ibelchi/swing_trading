...
def generate_opportunity_chart(symbol: str, hist_data: pd.DataFrame, metrics: dict) -> None:
    """
    ...
    past_earnings = metrics.get("past_earnings", [])
    
    print(f"[CHART {symbol}] next_earnings={next_earnings} past_earnings count={len(past_earnings)}")
    ...
    try:
        ...
        x_min = high_date - timedelta(days=20) # Rang visible
    except Exception:
